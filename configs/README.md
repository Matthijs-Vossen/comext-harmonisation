...
## Analysis Configs
- `configs/analysis/bilateral_persistence_cn2007_raw.yaml`: raw-data CN analogue of LT Table 3 around the `2006->2007` break, using zero-completed pair-code panels with a break-centered deterministic-all broad row, a filtered break-group broad row, and an adjusted row defined by the union of non-`1:1` break groups within that retained universe. It also writes companion aggregation-level diagnostics for bilateral, importer, and aggregate product shares.
...
- `configs/analysis/crm_revision_exposure_2023.yaml`: CN2023-anchored CRM application diagnostic showing cumulative revision exposure for unique CRM-related codes versus the full observed CN2023 code universe, with a separate threshold plot for codes touched by one or at least two allocation-requiring revisions.
...
- `configs/analysis/share_stability_deterministic_placebo.yaml`: share-stability companion with native deterministic-lineage placebo panels around the focal `2006->2007` break and the LT-converted focal panel retained.

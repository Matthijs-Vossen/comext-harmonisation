...
from ..common.metrics import mae_weighted, r2_45_weighted_symmetric
...
def _compute_chain_point(
    *,
    ...
    if (
        fixed_group_map is not None
        and fixed_group_ids is not None
        and fixed_sample_codes is not None
    ):
    ...
    if first_step.empty:
        raise ValueError(
            f"No step_index=1 metrics for chain {base_year}->{target_year}"
        )
...
def run_chain_length_analysis(config: ChainLengthConfig) -> dict[str, object]:
    ...
    for period, direction in sorted(required_steps):
        step_weights_cache[(period, direction, config.measures.weights_source)] = (
            load_step_weights(
                period=period,
                direction=direction,
        ...
        feasible_map_cache[(period, direction)] = feasible_target_map(
    ...
    summary["delta_mae_weighted"] = summary.groupby("direction")["mae_weighted"].diff()

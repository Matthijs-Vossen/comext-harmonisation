"""Build chained weights and apply them to monthly data with wide outputs."""
...
from comext_harmonisation.application import apply_chained_weights_wide_for_month_range
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Build chained weights and apply them to monthly data."
    )
    ...
    parser.add_argument(
        "--monthly-base-dir",
        type=str,
        default="data/extracted_no_confidential/products_like",
    ...
    parser.add_argument(
        "--no-skip-existing",
        action="store_true",
        help="Process months even if outputs/summary already exist.",
    )
...
def main() -> None:
    ...
    summary = apply_chained_weights_wide_for_month_range(
        start_year=args.start_year,
        end_year=args.end_year,
        target_year=args.target_year,
        measures=measures,
        monthly_base_dir=Path(args.monthly_base_dir),
